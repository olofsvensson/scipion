...
class CryoEMMetadataClient(object):

    def __init__(self, proposal, receivers, replyTo):
    ...
    def reportStackTrace(self):
        ...
        if len(self._receivers) > 0 and self._emailReplyTo is not None:
            COMMASPACE = ', '
            mime_text_message = MIMEText(errorMessage)
            replyTo = self._replyTo
            listTo = self._receivers
            listCC = []
            listBCC = []
